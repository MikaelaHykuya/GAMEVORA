...
export default function Store() {
    ...
    async (keyword = "") => {
      ...
      try {
        ...
        if (filter === "trending") query = query.eq("is_trending", true);
          ...
          query = query.eq("connectivity_type", filter);
        ...
        const releaseTimeStr = settings?.find(
          (s) => s.key === "release_time",
        )?.value;
        const now = new Date();
    ...
    const saleChannel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (game) {
            setLiveSales((prev) =>
              [{ title: game.title, time: Date.now() }, ...prev].slice(0, 5),
            );
            setTimeout(
              () =>
                setLiveSales((prev) =>
                  prev.filter((s) => s.time !== Date.now()),
                ),
              5000,
            );
          }
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0C] text-white overflow-hidden">
      <Helmet>
        <title>GVR - Store</title>
        <meta
          name="description"
          content="Browse GameVora's game catalog with filters, search, and exclusive deals. Find your next game today."
        />
      </Helmet>
        ...
        <div className="fixed bottom-24 right-4 z-[999] space-y-2 pointer-events-none">
          {liveSales.map((sale) => (
            ...
            >
              ...
              <p className="text-[11px] font-bold text-gray-200 mt-1">
                {sale.title}
              </p>
            </div>
          ))}
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-8">
              <div className="md:w-1/4 flex flex-col justify-center border-b md:border-b-0 md:border-r border-white/[0.08] pb-6 md:pb-0 md:pr-8">
                <div className="flex items-center gap-2 mb-4 bg-cyan-500/10 w-max px-3 py-1.5 rounded-full border border-cyan-500/20">
                  ...
                  <span className="text-cyan-400 text-[9px] font-black uppercase tracking-[0.4em] drop-shadow-[0_0_10px_rgba(34,211,238,0.8)]">
                    Network Live
                  </span>
                </div>
                <h3 className="text-2xl md:text-3xl font-black uppercase tracking-tight text-white mb-2 leading-none">
                  Vault News <br />
                ...
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mt-2">
                  Latest transmissions from GameVora HQ.
                </p>
              </div>
                    ...
                    >
                      ...
                      <div>
                        <div className="flex items-start justify-between mb-3 gap-2">
                          <span className="text-[9px] font-black bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 px-2.5 py-1 rounded-md uppercase tracking-widest shadow-[0_0_10px_rgba(34,211,238,0.2)]">
                            {item.category}
                          </span>
                        ...
                        <h4 className="text-sm font-black uppercase text-white mb-2 line-clamp-1 group-hover/item:text-cyan-300 transition-colors">
                          {item.title}
                        </h4>
      ...
      <main
        ref={storeRef}
        id="store"
        className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 py-0 mb-32"
      >
        {/* Header */}
        <ScrollReveal
          direction="down"
          className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6 mt-16 md:mt-0 relative"
        >
          <div className="absolute -top-32 -left-32 w-80 h-80 bg-purple-600/15 rounded-full blur-[120px] pointer-events-none" />
          <div className="space-y-5 relative z-10">
            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-xl shadow-lg shadow-black/50 cursor-default">
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-green-500"></span>
              </span>
              <span className="text-gray-300 text-[10px] md:text-xs font-black uppercase tracking-[0.3em]">
                Secure Connection Active
              </span>
            </div>
            <div>
              <h2 className="text-6xl md:text-7xl lg:text-[80px] font-black uppercase tracking-tighter leading-[0.9] relative">
                <span className="text-white drop-shadow-2xl">Vault</span>
                <br />
              ...
              <p className="text-xs md:text-sm text-gray-400 font-bold mt-4 tracking-widest uppercase flex items-center gap-4">
                ...
                {totalCount > 0
                  ? `${totalCount.toLocaleString("id-ID")} items decrypted and ready`
                  : "Awaiting decryption..."}
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 relative z-10">
          ...
          <div className="w-full lg:w-[280px] flex-shrink-0">
            <div className="lg:sticky lg:top-28 space-y-8 bg-[#0A0A0C]/80 backdrop-blur-3xl border border-white/[0.08] p-6 rounded-[2rem] shadow-2xl">
              {/* Search */}
              <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-purple-500 to-cyan-500 rounded-xl blur opacity-20 group-hover:opacity-40 transition duration-700"></div>
                <div className="relative flex items-center bg-zinc-900 border border-white/10 rounded-xl overflow-hidden">
                  <div className="pl-4 pr-2 flex items-center justify-center text-gray-400 group-hover:text-cyan-400 transition-colors">
                    <svg
                      className="w-4 h-4"
                      fill="none"
                      stroke="currentColor"
                  ...
                  <input
                    ...
                    placeholder="Search..."
                    className="w-full bg-transparent py-3 pr-4 outline-none text-xs font-black text-white placeholder:text-gray-600 tracking-wider uppercase"
                  />
                  {search && (
                    <button
                      onClick={() => {
                      ...
                      className="pr-4 text-gray-500 hover:text-red-400"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                  ...
                  {[
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] transition-all ${
                          isActive
                            ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.3)] border border-white/20"
                            : "bg-white/[0.03] border border-white/[0.05] text-gray-400 hover:text-white hover:border-white/[0.15] hover:bg-white/[0.06]"
                        }`}
                      >
                        {cat.label}
                      </button>
                  ...
                  {genres.map((g) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] transition-all ${
                          isActive
                            ? "bg-cyan-500/20 text-cyan-300 border border-cyan-500/50 shadow-[0_0_15px_rgba(34,211,238,0.2)]"
                            : "bg-white/[0.03] border border-white/[0.05] text-gray-400 hover:text-cyan-400 hover:border-cyan-500/30 hover:bg-white/[0.06]"
                        }`}
                      >
                        {g}
                        {isActive && (
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5 min-h-[400px]">
              {loading ? (
                <>
                  {Array.from({ length: 12 }).map((_, i) => (
                    <ScrollReveal key={i} delay={(i % 4) * 0.1} direction="up">
                      <GameCardSkeleton />
                    </ScrollReveal>
                  ))}
                </>
                ...
                <div className="col-span-full text-center py-32 bg-black/20 border border-white/5 rounded-3xl backdrop-blur-sm shadow-inner relative overflow-hidden">
                  ...
                  <div className="relative w-24 h-24 mx-auto mb-6">
                    ...
                    <svg
                      className="relative w-10 h-10 text-red-500 mx-auto mt-[28px]"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  <p className="text-white text-2xl font-black uppercase tracking-widest drop-shadow-md">
                    {search ? "Signal Lost" : "Vault Empty"}
                  </p>
                  <p className="text-gray-400 text-sm font-medium mt-3 max-w-md mx-auto leading-relaxed">
                    {search
                      ? `Tidak ada data game yang cocok dengan pencarian "${search}". Silakan gunakan kata kunci lain.`
                      : "Belum ada game yang tersedia di toko."}
                  </p>
                  {search && (
                    <button
                      onClick={() => {
                ...
                games.map((game, i) => (
                  ...
                  >
                    <div className="relative">
                      <div
                        className={`absolute z-20 flex items-center gap-1 px-2.5 py-1 rounded-lg text-[7px] font-black uppercase tracking-wider border backdrop-blur-sm ${
                      ...
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${game.connectivity_type === "Online" ? "bg-blue-400" : "bg-green-400"} animate-pulse`}
                        />
            ...
            <div className="relative w-full max-w-md bg-zinc-900/90 border border-purple-500/30 rounded-[32px] p-8 text-center shadow-[0_0_50px_rgba(168,85,247,0.3)] overflow-hidden animate-fade-up">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-gray-400 leading-relaxed mb-8 px-4 font-sans">
                  Koneksi ke server utama berhasil didirikan. Selamat
                  mengeksplorasi dan berburu di brankas digital ini!
                </p>
