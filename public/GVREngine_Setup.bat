...
echo f.WriteLine "if ($Uri -match 's=([^&]+)&l=([^&]+)&a=([^&/]+)(?:&n=([^&]+))?') {" >> "%temp%\gen.vbs"
...
echo f.WriteLine "    $gameName   = if ($matches[4]) { [System.Uri]::UnescapeDataString($(FromB64 (Unescape $matches[4]))) } else { 'Game' }" >> "%temp%\gen.vbs"
...
echo f2.WriteLine "    Set objShell = CreateObject(""Shell.Application"")" >> "%temp%\gen.vbs"
echo f2.WriteLine "    objShell.ShellExecute ""powershell.exe"", ""-ExecutionPolicy Bypass -File """"C:\GVREngine\handler.ps1"""" """""" & objArgs(0) & """""""", """", ""runas"", 1" >> "%temp%\gen.vbs"
